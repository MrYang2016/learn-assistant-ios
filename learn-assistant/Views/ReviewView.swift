...
struct ReviewView: View {
  ...
  var body: some View {
    NavigationView {
      ZStack {
        if viewModel.isLoading && viewModel.currentReview == nil && viewModel.totalCount == 0 {
          ProgressView()
        } else if viewModel.currentReview == nil && viewModel.totalCount == 0 {
          emptyStateView
        } else if viewModel.currentIndex >= viewModel.totalCount {
          completedView
        } else {
          reviewCardView
        }
      ...
      .task {
        if viewModel.currentReview == nil && viewModel.totalCount == 0 {
  ...
  private var emptyStateView: some View {
    VStack(spacing: 20) {
      ...
      Text("Add knowledge points with review enabled to start building your review schedule")
        .font(.caption)
        .foregroundColor(.secondary)
        .multilineTextAlignment(.center)
        .padding(.horizontal)
